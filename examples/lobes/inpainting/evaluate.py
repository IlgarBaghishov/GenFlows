"""Evaluate inpainting models by round-trip consistency with a pretrained CNN.

For each test sample: create inpainting masks, generate inpainted structures,
then run the CNN to predict properties back. Compare predicted vs. original conditioning.
"""
...
def make_masks(scenario, n, device):
    """Generate a batch of masks for a given scenario."""
    generators = {
        'wells': generate_well_mask,
        'boundaries': generate_boundary_mask,
        'cross_section': generate_cross_section_mask,
        'combo_wells_bounds': lambda: generate_combination_mask((50, 50, 50), ['wells', 'boundaries']),
        'combo_all': lambda: generate_combination_mask((50, 50, 50), ['wells', 'boundaries', 'cross_sections']),
    }
    gen = generators[scenario]
    return torch.stack([gen() for _ in range(n)]).to(device)


def main():
    parser = argparse.ArgumentParser(description='Evaluate inpainting models via CNN round-trip')
    ...
    parser.add_argument('--scenarios', type=str, nargs='+',
                        default=['wells', 'boundaries', 'cross_section', 'combo_wells_bounds', 'combo_all'],
                        help='Mask scenarios to evaluate')
    ...
    # Collect test data: facies (ground truth) and conditioning
    all_facies = []
    all_cond_norm = []
    for facies, cond in test_loader:
        all_facies.append(facies)
        all_cond_norm.append(cond)
        if sum(c.shape[0] for c in all_cond_norm) >= args.n_samples:
            break
    all_facies = torch.cat(all_facies)[:args.n_samples]
    ...
    # Evaluate each method x scenario
    for method_name, method in methods.items():
        for scenario in args.scenarios:
            ...
            for i in range(0, n_samples, args.batch_size):
                ...
                batch_facies = all_facies[i:batch_end].to(device)  # (B, 1, 50, 50, 50) in [-1,1]
                batch_cond = all_cond_norm[i:batch_end].to(device)
                batch_size = batch_cond.shape[0]
                shape = (batch_size, 1, 50, 50, 50)

                # Generate masks and known data
                mask = make_masks(scenario, batch_size, device)
                known_data = batch_facies * mask
            ...
            all_pred['angle_deg'] = resolve_angle_ambiguity(
                all_pred['angle_deg'], targets['angle_deg']
            )
            ...
            # Save data
            np.savez(
                f"results/eval_{method_name}_{scenario}_{args.n_steps}steps.npz",
            ...
            plot_parity(
                ...
                save_path=f"results/parity_{method_name}_{scenario}_{args.n_steps}steps.png",
                title_prefix=f"{method_name} / {scenario}",
            )
